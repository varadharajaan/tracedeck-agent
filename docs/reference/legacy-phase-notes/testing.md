...
The modern console verifier reruns the redesigned top-navigation dashboard and
Browser Intelligence UI gates end to end:

```powershell
python ./devctl.py test modern-ui
powershell -NoProfile -ExecutionPolicy Bypass -File ./scripts/verify/verify-modern-console-ui.ps1
```

It checks Python syntax, Go formatting, dashboard DOM ids, dashboard and
Browser Intelligence JavaScript, live boot on `127.0.0.1:18080`, screenshot-free
layout/theme/visual metrics, browser badge integrity, live provenance, root
artifact hygiene, and `git diff --check`. The UI defaults to live evidence and
hides demo proof rows until the demo proof toggle is enabled. The verifier also
checks that `/v1-old` serves the preserved legacy dashboard fallback. The
layout and visual-quality checks wait through the post-load async data render
before measuring page width, so delayed right-side widening is treated as a
test failure on desktop, tablet, and mobile viewports.
Phase 81 extends that contract to require product-grade Workspace Navigator
...
Phase 81 adds Workspace Navigator clarity:
...
python ./devctl.py test phase81
```

The Phase 81 verifier compiles `devctl.py` and the dashboard visual checker,
...
Live provider implementation checks:
...
powershell -NoProfile -ExecutionPolicy Bypass -File ./scripts/verify/verify-phase113.ps1
```

`test-email-notifier.ps1` covers SES/SMTP notifier wiring, Web Push provider
payload construction, channel severity filtering, and app runner delivery
behavior. `test-backend-api.ps1` covers the dashboard service worker asset and
Web Push subscription endpoints. The VAPID generator writes keys under
`data/local/webpush` and logs under `logs/local/webpush`. Phase 113 verifies
the service worker, public-key endpoint, subscription endpoint, privacy-safe
subscription response, and Newman collection. The smoke/Newman scripts back up
and restore existing Web Push state, including the real browser subscription
file and VAPID keys, so provider tests do not silently rotate live push
credentials.
