...
Initialize-TraceDeckScriptLog -Name "verify-phase54" -LogRoot "logs/local/verify" | Out-Null
...
try {
    ...
    Invoke-TraceDeckLoggedCommand -Label "Dashboard JavaScript syntax check" -Command {
        powershell -NoProfile -ExecutionPolicy Bypass -File ./scripts/local/test-dashboard-js.ps1 -OutputRoot "data/local/dashboard-js-check/phase54"
    }
    Invoke-TraceDeckLoggedCommand -Label "Phase 54 notification revenue cockpit smoke" -Command {
        powershell -NoProfile -ExecutionPolicy Bypass -File ./scripts/local/smoke-phase54.ps1
    }
    Invoke-TraceDeckLoggedCommand -Label "Phase 54 Newman collection" -Command {
    ...
    Invoke-TraceDeckLoggedCommand -Label "Cross-platform build verification" -Command {
        powershell -NoProfile -ExecutionPolicy Bypass -File ./scripts/verify/check-cross-platform-build.ps1 -BuildRoot "data/local/build/phase54"
    }
