...
Initialize-TraceDeckScriptLog -Name "verify-phase93" -LogRoot "logs/local/verify" | Out-Null
...
try {
    ...
    Invoke-TraceDeckLoggedCommand -Label "Phase 93 task advisory smoke" -Command {
        powershell -NoProfile -ExecutionPolicy Bypass -File ./scripts/local/smoke-phase93.ps1
    }
    Invoke-TraceDeckLoggedCommand -Label "Phase 93 Newman collection" -Command {
