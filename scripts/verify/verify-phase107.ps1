...
Initialize-TraceDeckScriptLog -Name "verify-phase107" -LogRoot "logs/local/verify" | Out-Null
...
try {
    ...
    Invoke-TraceDeckLoggedCommand -Label "Phase ledger contract" -Command {
        powershell -NoProfile -ExecutionPolicy Bypass -File ./scripts/local/test-phase-ledger.ps1
    }
    Invoke-TraceDeckLoggedCommand -Label "Contract completion audit" -Command {
        powershell -NoProfile -ExecutionPolicy Bypass -File ./scripts/local/test-contract-completion-audit.ps1
    }
    Invoke-TraceDeckLoggedCommand -Label "Refresh contract audit output" -Command {
