...
Initialize-TraceDeckScriptLog -Name "verify-phase109" -LogRoot "logs/local/verify" | Out-Null
...
try {
    ...
    Invoke-TraceDeckLoggedCommand -Label "OpenTelemetry exporter and stack contract" -Command {
    ...
    Invoke-TraceDeckLoggedCommand -Label "Phase 109 OTLP smoke" -Command {
        powershell -NoProfile -ExecutionPolicy Bypass -File ./scripts/local/smoke-phase109.ps1
    }
    Invoke-TraceDeckLoggedCommand -Label "Phase 109 Newman" -Command {
