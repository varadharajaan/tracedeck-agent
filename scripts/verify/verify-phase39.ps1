...
Initialize-TraceDeckScriptLog -Name "verify-phase39" -LogRoot "logs/local/verify" | Out-Null
...
try {
    ...
    Invoke-TraceDeckLoggedCommand -Label "Phase 39 autostart assurance" -Command {
    ...
    Invoke-TraceDeckLoggedCommand -Label "Windows task template smoke" -Command {
    ...
    Invoke-TraceDeckLoggedCommand -Label "Dashboard JavaScript syntax check" -Command {
        powershell -NoProfile -ExecutionPolicy Bypass -File ./scripts/local/test-dashboard-js.ps1 -OutputRoot "data/local/dashboard-js-check/phase39"
    }

    Invoke-TraceDeckLoggedCommand -Label "Phase 39 service/autostart smoke" -Command {
        powershell -NoProfile -ExecutionPolicy Bypass -File ./scripts/local/smoke-phase39.ps1
    }

    Invoke-TraceDeckLoggedCommand -Label "Phase 39 Newman collection" -Command {
    ...
    Invoke-TraceDeckLoggedCommand -Label "Cross-platform build verification" -Command {
        powershell -NoProfile -ExecutionPolicy Bypass -File ./scripts/verify/check-cross-platform-build.ps1 -BuildRoot "data/local/build/phase39"
    }
