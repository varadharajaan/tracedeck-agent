...
Initialize-TraceDeckScriptLog -Name "verify-phase78" -LogRoot "logs/local/verify" | Out-Null
...
try {
    ...
    Invoke-TraceDeckLoggedCommand -Label "Compile devctl and dashboard UI checkers" -Command {
        python -B -m py_compile ./devctl.py ./scripts/tools/dashboard_theme_check.py ./scripts/tools/dashboard_visual_quality_check.py
    }
    ...
    Invoke-TraceDeckLoggedCommand -Label "Dashboard JavaScript syntax check" -Command {
        powershell -NoProfile -ExecutionPolicy Bypass -File ./scripts/local/test-dashboard-js.ps1 -OutputRoot "data/local/dashboard-js-check/phase78/dashboard"
    }
    Invoke-TraceDeckLoggedCommand -Label "Browser activity JavaScript syntax check" -Command {
        powershell -NoProfile -ExecutionPolicy Bypass -File ./scripts/local/test-dashboard-js.ps1 -DashboardPath "backend/internal/api/web/browser_activity.html" -OutputRoot "data/local/dashboard-js-check/phase78/browser-activity"
    }
    ...
    Invoke-TraceDeckLoggedCommand -Label "Phase 78 provider setup smoke" -Command {
        powershell -NoProfile -ExecutionPolicy Bypass -File ./scripts/local/smoke-phase78.ps1
    }
    Invoke-TraceDeckLoggedCommand -Label "Phase 78 Newman collection" -Command {
        powershell -NoProfile -ExecutionPolicy Bypass -File ./scripts/local/newman-phase78.ps1
    }
