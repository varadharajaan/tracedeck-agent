...
Initialize-TraceDeckScriptLog -Name "smoke-phase50" -LogRoot "logs/local/smoke" | Out-Null

$timestamp = Get-Date -Format "yyyyMMdd-HHmmss"
$smokeRoot = "data/local/smoke-phase50/$timestamp"
$pidPath = "$smokeRoot/tracedeck-backend.pid"
$dataPath = "$smokeRoot/backend-state.json"
$layoutRoot = "data/local/dashboard-layout/phase50-smoke/$timestamp"
...
try {
    ...
    Write-TraceDeckLog -Level "INFO" -Message "Phase 50 business dashboard smoke passed addr=$Addr score=$($business.summary.product_score) alerts=$($business.alerts.Count)"
