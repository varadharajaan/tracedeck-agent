...
Initialize-TraceDeckScriptLog -Name "newman-phase14" -LogRoot "logs/local/newman" | Out-Null
...
try {
    ...
    $runRoot = Join-Path $script:TraceDeckRepoRoot "data/local/newman/phase14/$timestamp"
    ...
    $baseUrl = "http://$Addr"
    Invoke-TraceDeckLoggedCommand -Label "Build backend Phase 14 Newman executable" -Command {
    ...
    $backend = Start-Process -FilePath $exePath -ArgumentList @(
        ...
        "--api-key-actor-id", "phase14-newman"
    ...
    Write-TraceDeckLog -Level "INFO" -Message "Started backend for Phase 14 Newman pid=$($backend.Id) addr=$Addr"

    Wait-TraceDeckBackend -BaseUrl $baseUrl

    Invoke-TraceDeckLoggedCommand -Label "Run Newman Phase 14 collection" -Command {
        newman run ./postman/tracedeck-backend-phase14.postman_collection.json --env-var "baseUrl=$baseUrl" --env-var "apiKey=$ApiKey" --reporters "cli,json" --reporter-json-export $reportPath
    }
    ...
    Write-TraceDeckLog -Level "INFO" -Message "Newman Phase 14 report: $reportPath"
    ...
    if ($backend -and -not $backend.HasExited) {
        ...
        Write-TraceDeckLog -Level "INFO" -Message "Stopped backend Phase 14 Newman process: $($backend.Id)"
    }
}
