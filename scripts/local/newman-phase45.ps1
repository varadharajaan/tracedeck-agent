...
Initialize-TraceDeckScriptLog -Name "newman-phase45" -LogRoot "logs/local/newman" | Out-Null

$timestamp = Get-Date -Format "yyyyMMdd-HHmmss"
$runRoot = "data/local/newman/phase45/$timestamp"
...
try {
    ...
    Invoke-TraceDeckLoggedCommand -Label "Run Newman Phase 45 collection" -Command {
        newman run ./postman/tracedeck-backend-phase45.postman_collection.json --env-var "baseUrl=$baseUrl" --reporters "cli,json" --reporter-json-export $reportPath
    }
    ...
    Write-TraceDeckLog -Level "INFO" -Message "Newman Phase 45 report: $reportPath"
