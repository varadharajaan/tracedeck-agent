...
Initialize-TraceDeckScriptLog -Name "newman-phase40" -LogRoot "logs/local/newman" | Out-Null

$timestamp = Get-Date -Format "yyyyMMdd-HHmmss"
$runRoot = "data/local/newman/phase40/$timestamp"
...
try {
    ...
    Invoke-TraceDeckLoggedCommand -Label "Run Newman Phase 40 collection" -Command {
        newman run ./postman/tracedeck-backend-phase40.postman_collection.json --env-var "baseUrl=$baseUrl" --reporters "cli,json" --reporter-json-export $reportPath
    }
    ...
    Write-TraceDeckLog -Level "INFO" -Message "Newman Phase 40 report: $reportPath"
