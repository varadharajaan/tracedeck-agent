...
Initialize-TraceDeckScriptLog -Name "newman-phase86" -LogRoot "logs/local/newman" | Out-Null

$timestamp = Get-Date -Format "yyyyMMdd-HHmmss"
$runRoot = "data/local/newman/phase86/$timestamp"
...
try {
    ...
    Invoke-TraceDeckLoggedCommand -Label "Run Newman Phase 86 collection" -Command {
        newman run ./postman/tracedeck-backend-phase86.postman_collection.json --env-var "baseUrl=$baseUrl" --reporters "cli,json" --reporter-json-export $reportPath
    }
    ...
    Write-TraceDeckLog -Level "INFO" -Message "Newman Phase 86 report: $reportPath"
