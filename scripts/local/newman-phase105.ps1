...
Initialize-TraceDeckScriptLog -Name "newman-phase105" -LogRoot "logs/local/newman" | Out-Null

$timestamp = Get-Date -Format "yyyyMMdd-HHmmss"
$runRoot = "data/local/newman/phase105/$timestamp"
...
$promotionPath = "$runRoot/promotion-readiness.json"
$promotionTextPath = "$runRoot/promotion-readiness.txt"
...
try {
    ...
    Invoke-TraceDeckLoggedCommand -Label "Generate Phase 105 Newman runtime summary" -Command {
        powershell -NoProfile -ExecutionPolicy Bypass -File ./scripts/local/get-runtime-summary.ps1 `
            -Addr $Addr `
            -TaskName "\TraceDeck\TraceDeck Phase105 Newman Missing" `
            -PidPath $pidPath `
            -ReadyPath $readyPath `
            -TaskStatusOutputPath $taskStatusPath
    }
    Invoke-TraceDeckLoggedCommand -Label "Generate Phase 105 Newman verification evidence" -Command {
        powershell -NoProfile -ExecutionPolicy Bypass -File ./scripts/local/get-verification-evidence.ps1 `
            -Phase "phase105" `
            -BaseUrl $baseUrl
    }
    Invoke-TraceDeckLoggedCommand -Label "Generate Phase 105 Newman operator assurance" -Command {
    ...
    Invoke-TraceDeckLoggedCommand -Label "Generate Phase 105 Newman promotion readiness" -Command {
    ...
    Invoke-TraceDeckLoggedCommand -Label "Run Newman Phase 105 collection" -Command {
        newman run ./postman/tracedeck-backend-phase105.postman_collection.json --env-var "baseUrl=$baseUrl" --reporters "cli,json" --reporter-json-export $reportPath
    }
    ...
    Write-TraceDeckLog -Level "INFO" -Message "Newman Phase 105 report: $reportPath"
