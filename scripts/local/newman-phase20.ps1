...
Initialize-TraceDeckScriptLog -Name "newman-phase20" -LogRoot "logs/local/newman" | Out-Null
...
try {
    ...
    $runRoot = Join-Path $script:TraceDeckRepoRoot "data/local/newman/phase20/$timestamp"
    ...
    $baseUrl = "http://$Addr"
    Invoke-TraceDeckLoggedCommand -Label "Build backend Phase 20 Newman executable" -Command {
    ...
    $backend = Start-Process -FilePath $exePath -ArgumentList @(
        ...
        "--api-key-actor-id", "phase20-newman"
    ...
    Write-TraceDeckLog -Level "INFO" -Message "Started backend for Phase 20 Newman pid=$($backend.Id) addr=$Addr"

    Wait-TraceDeckBackend -BaseUrl $baseUrl

    Invoke-TraceDeckLoggedCommand -Label "Run Newman Phase 20 collection" -Command {
        newman run ./postman/tracedeck-backend-phase20.postman_collection.json --env-var "baseUrl=$baseUrl" --env-var "apiKey=$ApiKey" --reporters "cli,json" --reporter-json-export $reportPath
    }
    ...
    Write-TraceDeckLog -Level "INFO" -Message "Newman Phase 20 report: $reportPath"
    ...
    if ($backend -and -not $backend.HasExited) {
        ...
        Write-TraceDeckLog -Level "INFO" -Message "Stopped backend Phase 20 Newman process: $($backend.Id)"
    }
}
