...
Initialize-TraceDeckScriptLog -Name "newman-phase99" -LogRoot "logs/local/newman" | Out-Null

$timestamp = Get-Date -Format "yyyyMMdd-HHmmss"
$runRoot = "data/local/newman/phase99/$timestamp"
...
try {
    ...
    Invoke-TraceDeckLoggedCommand -Label "Generate Phase 99 Newman runtime summary" -Command {
        powershell -NoProfile -ExecutionPolicy Bypass -File ./scripts/local/get-runtime-summary.ps1 `
            -Addr $Addr `
            -TaskName "\TraceDeck\TraceDeck Phase99 Newman Missing" `
            -PidPath $pidPath `
            -ReadyPath $readyPath `
            -TaskStatusOutputPath $taskStatusPath
    }

    Invoke-TraceDeckLoggedCommand -Label "Generate Phase 99 Newman verification evidence" -Command {
    ...
    Invoke-TraceDeckLoggedCommand -Label "Run Newman Phase 99 collection" -Command {
        newman run ./postman/tracedeck-backend-phase99.postman_collection.json --env-var "baseUrl=$baseUrl" --reporters "cli,json" --reporter-json-export $reportPath
    }
    ...
    Write-TraceDeckLog -Level "INFO" -Message "Newman Phase 99 report: $reportPath"
