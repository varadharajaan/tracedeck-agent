...
Initialize-TraceDeckScriptLog -Name "newman-phase9" -LogRoot "logs/local/newman" | Out-Null
...
try {
    ...
    $runRoot = Join-Path $script:TraceDeckRepoRoot "data/local/newman/phase9/$timestamp"
    ...
    $baseUrl = "http://$Addr"
    Invoke-TraceDeckLoggedCommand -Label "Build backend Phase 9 Newman executable" -Command {
    ...
    Write-TraceDeckLog -Level "INFO" -Message "Started backend for Phase 9 Newman pid=$($backend.Id) addr=$Addr"

    Wait-TraceDeckBackend -BaseUrl $baseUrl

    Invoke-TraceDeckLoggedCommand -Label "Run Newman Phase 9 collection" -Command {
        newman run ./postman/tracedeck-backend-phase9.postman_collection.json --env-var "baseUrl=$baseUrl" --reporters "cli,json" --reporter-json-export $reportPath
    }
    ...
    Write-TraceDeckLog -Level "INFO" -Message "Newman Phase 9 report: $reportPath"
    ...
    if ($backend -and -not $backend.HasExited) {
        ...
        Write-TraceDeckLog -Level "INFO" -Message "Stopped backend Phase 9 Newman process: $($backend.Id)"
    }
}
