...
Initialize-TraceDeckScriptLog -Name "smoke-phase47" -LogRoot "logs/local/smoke" | Out-Null

$timestamp = Get-Date -Format "yyyyMMdd-HHmmss"
$smokeRoot = "data/local/smoke-phase47/$timestamp"
$pidPath = "$smokeRoot/tracedeck-backend.pid"
$dataPath = "$smokeRoot/backend-state.json"
$layoutRoot = "data/local/dashboard-layout/phase47-smoke/$timestamp"
...
try {
    ...
    foreach ($expected in @(
        ...
        if ($dashboard.Content -notmatch [regex]::Escape($expected)) {
            throw "Expected Phase 47 dashboard marker '$expected'."
        }
    }
    ...
    if ($commandCenter.summary.open_alerts -lt 1 -or $commandCenter.channels.Count -lt 3 -or $commandCenter.actions.Count -lt 1) {
        throw "Expected premium notification command center to expose alerts, routes, and actions."
    }
    ...
    Write-TraceDeckLog -Level "INFO" -Message "Phase 47 premium notification smoke passed addr=$Addr alerts=$($commandCenter.summary.open_alerts) routes=$($commandCenter.channels.Count)"
