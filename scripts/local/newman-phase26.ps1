...
Initialize-TraceDeckScriptLog -Name "newman-phase26" -LogRoot "logs/local/newman" | Out-Null

$timestamp = Get-Date -Format "yyyyMMdd-HHmmss"
$runRoot = "data/local/newman/phase26/$timestamp"
...
try {
    ...
    Wait-TraceDeckNotificationRoutes -BaseUrl $baseUrl

    Invoke-TraceDeckLoggedCommand -Label "Run Newman Phase 26 collection" -Command {
        newman run ./postman/tracedeck-backend-phase26.postman_collection.json --env-var "baseUrl=$baseUrl" --reporters "cli,json" --reporter-json-export $reportPath
    }
    ...
    Write-TraceDeckLog -Level "INFO" -Message "Newman Phase 26 report: $reportPath"
