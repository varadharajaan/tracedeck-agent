...
Initialize-TraceDeckScriptLog -Name "smoke-phase28" -LogRoot "logs/local/smoke" | Out-Null

$timestamp = Get-Date -Format "yyyyMMdd-HHmmss"
$smokeRoot = "data/local/smoke-phase28/$timestamp"
...
try {
    ...
    Invoke-TraceDeckLoggedCommand -Label "Run agent once with backend sync" -Command {
    ...
    if ($status.stored_events -lt 1) {
        throw "Expected backend telemetry status to include synced agent events."
    }
    ...
    foreach ($expected in @("Live Agent Telemetry", "Telemetry Privacy Boundary")) {
    ...
    Write-TraceDeckLog -Level "INFO" -Message "Phase 28 agent telemetry ingest smoke passed addr=$Addr stored_events=$($status.stored_events)"
