...
Initialize-TraceDeckScriptLog -Name "newman-phase109" -LogRoot "logs/local/newman" | Out-Null

$timestamp = Get-Date -Format "yyyyMMdd-HHmmss"
$runRoot = "data/local/newman/phase109/$timestamp"
...
try {
    ...
    Invoke-TraceDeckLoggedCommand -Label "Run Newman Phase 109 collection" -Command {
        newman run ./postman/tracedeck-backend-phase109.postman_collection.json --env-var "baseUrl=$baseUrl" --reporters "cli,json" --reporter-json-export $reportPath
    }
    ...
    Write-TraceDeckLog -Level "INFO" -Message "Newman Phase 109 report: $reportPath"
