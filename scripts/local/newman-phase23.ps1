...
Initialize-TraceDeckScriptLog -Name "newman-phase23" -LogRoot "logs/local/newman" | Out-Null
...
try {
    ...
    $runRoot = Join-Path $script:TraceDeckRepoRoot "data/local/newman/phase23/$timestamp"
    ...
    $baseUrl = "http://$Addr"
    Invoke-TraceDeckLoggedCommand -Label "Build backend Phase 23 Newman executable" -Command {
    ...
    $backend = Start-Process -FilePath $exePath -ArgumentList @(
        ...
        "--api-key-actor-id", "phase23-newman"
    ...
    Write-TraceDeckLog -Level "INFO" -Message "Started backend for Phase 23 Newman pid=$($backend.Id) addr=$Addr"
    Wait-TraceDeckBackend -BaseUrl $baseUrl

    Invoke-TraceDeckLoggedCommand -Label "Run Newman Phase 23 collection" -Command {
        newman run ./postman/tracedeck-backend-phase23.postman_collection.json --env-var "baseUrl=$baseUrl" --env-var "apiKey=$ApiKey" --reporters "cli,json" --reporter-json-export $reportPath
    }
    ...
    Write-TraceDeckLog -Level "INFO" -Message "Newman Phase 23 report: $reportPath"
    ...
    if ($backend -and -not $backend.HasExited) {
        ...
        Write-TraceDeckLog -Level "INFO" -Message "Stopped backend Phase 23 Newman process: $($backend.Id)"
    }
}
