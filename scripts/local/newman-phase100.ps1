...
Initialize-TraceDeckScriptLog -Name "newman-phase100" -LogRoot "logs/local/newman" | Out-Null

$timestamp = Get-Date -Format "yyyyMMdd-HHmmss"
$runRoot = "data/local/newman/phase100/$timestamp"
...
try {
    ...
    Invoke-TraceDeckLoggedCommand -Label "Generate Phase 100 Newman runtime summary" -Command {
        powershell -NoProfile -ExecutionPolicy Bypass -File ./scripts/local/get-runtime-summary.ps1 `
            -Addr $Addr `
            -TaskName "\TraceDeck\TraceDeck Phase100 Newman Missing" `
            -PidPath $pidPath `
            -ReadyPath $readyPath `
            -TaskStatusOutputPath $taskStatusPath
    }

    Invoke-TraceDeckLoggedCommand -Label "Generate Phase 100 Newman verification evidence" -Command {
        powershell -NoProfile -ExecutionPolicy Bypass -File ./scripts/local/get-verification-evidence.ps1 `
            -Phase "phase100" `
            -BaseUrl $baseUrl
    }

    Invoke-TraceDeckLoggedCommand -Label "Generate Phase 100 Newman operator assurance" -Command {
    ...
    Invoke-TraceDeckLoggedCommand -Label "Run Newman Phase 100 collection" -Command {
        newman run ./postman/tracedeck-backend-phase100.postman_collection.json --env-var "baseUrl=$baseUrl" --reporters "cli,json" --reporter-json-export $reportPath
    }
    ...
    Write-TraceDeckLog -Level "INFO" -Message "Newman Phase 100 report: $reportPath"
