...
Initialize-TraceDeckScriptLog -Name "newman-phase27" -LogRoot "logs/local/newman" | Out-Null

$timestamp = Get-Date -Format "yyyyMMdd-HHmmss"
$runRoot = "data/local/newman/phase27/$timestamp"
...
try {
    ...
    Wait-TraceDeckSeededDevices -BaseUrl $baseUrl

    Invoke-TraceDeckLoggedCommand -Label "Run Newman Phase 27 collection" -Command {
        newman run ./postman/tracedeck-backend-phase27.postman_collection.json --env-var "baseUrl=$baseUrl" --reporters "cli,json" --reporter-json-export $reportPath
    }
    ...
    Write-TraceDeckLog -Level "INFO" -Message "Newman Phase 27 report: $reportPath"
