...
Initialize-TraceDeckScriptLog -Name "newman-phase80" -LogRoot "logs/local/newman" | Out-Null
...
try {
    ...
    $runRoot = "data/local/newman/phase80/$timestamp"
    ...
    Invoke-TraceDeckLoggedCommand -Label "Upload Phase 80 cloud sample before Newman" -Command {
        $uploadArgs = @(
            "-NoProfile",
    ...
    Invoke-TraceDeckLoggedCommand -Label "Run Newman Phase 80 cloud collection" -Command {
        newman run ./postman/tracedeck-cloud-phase80.postman_collection.json --env-var "lambdaUrl=$FrontendUrl" --reporters "cli,json" --reporter-json-export $reportPath
    }

    Write-TraceDeckLog -Level "INFO" -Message "Newman Phase 80 report: $reportPath"
