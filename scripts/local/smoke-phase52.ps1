...
Initialize-TraceDeckScriptLog -Name "smoke-phase52" -LogRoot "logs/local/smoke" | Out-Null

$timestamp = Get-Date -Format "yyyyMMdd-HHmmss"
$smokeRoot = "data/local/smoke-phase52/$timestamp"
$pidPath = "$smokeRoot/tracedeck-backend.pid"
$dataPath = "$smokeRoot/backend-state.json"
$layoutRoot = "data/local/dashboard-layout/phase52-smoke/$timestamp"
...
try {
    ...
    $roles = Invoke-RestMethod -Method "GET" -Uri "$baseUrl/api/v1/tenants/$tenantID/role-experiences"
    if ($roles.summary.readiness_score -le 0 -or $roles.summary.roles_total -ne 4 -or $roles.roles.Count -ne 4 -or $roles.onboarding.Count -lt 4) {
        throw "Expected role experiences to expose four scored role views and onboarding actions."
    }
    foreach ($expectedRole in @("parent", "student", "school_admin", "business_manager")) {
        if (-not ($roles.roles | Where-Object { $_.role_id -eq $expectedRole })) {
            throw "Expected role experience for $expectedRole."
        }
    }
    if ($roles.privacy_boundary -notmatch "metadata-only" -or $roles.privacy_boundary -notmatch "no passwords") {
        throw "Expected strict role experience privacy boundary."
    }
    $roleSerialized = ($roles | ConvertTo-Json -Depth 24).ToLowerInvariant()
    foreach ($forbidden in @("smtp_password", "provider_secret", "screenshot_bytes", "raw_url", "alert_body")) {
        if ($roleSerialized.Contains($forbidden)) {
            throw "Role experiences leaked forbidden field marker '$forbidden'."
        }
    }
    ...
    Write-TraceDeckLog -Level "INFO" -Message "Phase 52 role experience smoke passed addr=$Addr score=$($roles.summary.readiness_score) roles=$($roles.roles.Count)"
