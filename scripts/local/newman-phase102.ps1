...
Initialize-TraceDeckScriptLog -Name "newman-phase102" -LogRoot "logs/local/newman" | Out-Null

$timestamp = Get-Date -Format "yyyyMMdd-HHmmss"
$runRoot = "data/local/newman/phase102/$timestamp"
...
try {
    ...
    Invoke-TraceDeckLoggedCommand -Label "Generate Phase 102 Newman runtime summary with stale ready PID" -Command {
        powershell -NoProfile -ExecutionPolicy Bypass -File ./scripts/local/get-runtime-summary.ps1 `
            -Addr $Addr `
            -TaskName "\TraceDeck\TraceDeck Phase102 Newman Missing" `
            -PidPath $pidPath `
            -ReadyPath $readyPath `
            -TaskStatusOutputPath $taskStatusPath
    }

    Invoke-TraceDeckLoggedCommand -Label "Generate Phase 102 Newman verification evidence" -Command {
        powershell -NoProfile -ExecutionPolicy Bypass -File ./scripts/local/get-verification-evidence.ps1 `
            -Phase "phase102" `
            -BaseUrl $baseUrl
    }

    Invoke-TraceDeckLoggedCommand -Label "Generate Phase 102 Newman operator assurance" -Command {
    ...
    Invoke-TraceDeckLoggedCommand -Label "Run Newman Phase 102 collection" -Command {
        newman run ./postman/tracedeck-backend-phase102.postman_collection.json --env-var "baseUrl=$baseUrl" --reporters "cli,json" --reporter-json-export $reportPath
    }
    ...
    Write-TraceDeckLog -Level "INFO" -Message "Newman Phase 102 report: $reportPath"
