...
Initialize-TraceDeckScriptLog -Name "newman-phase92" -LogRoot "logs/local/newman" | Out-Null

$timestamp = Get-Date -Format "yyyyMMdd-HHmmss"
$runRoot = "data/local/newman/phase92/$timestamp"
...
try {
    ...
    Invoke-TraceDeckLoggedCommand -Label "Run Newman Phase 92 collection" -Command {
        newman run ./postman/tracedeck-backend-phase92.postman_collection.json --env-var "baseUrl=$baseUrl" --reporters "cli,json" --reporter-json-export $reportPath
    }
    ...
    Write-TraceDeckLog -Level "INFO" -Message "Newman Phase 92 report: $reportPath"
