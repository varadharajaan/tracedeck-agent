...
Initialize-TraceDeckScriptLog -Name "newman-phase112" -LogRoot "logs/local/newman" | Out-Null

$timestamp = Get-Date -Format "yyyyMMdd-HHmmss"
$runRoot = "data/local/newman/phase112/$timestamp"
...
try {
    ...
    Invoke-TraceDeckLoggedCommand -Label "Generate Phase 112 runtime summary" -Command {
        powershell -NoProfile -ExecutionPolicy Bypass -File ./scripts/local/get-runtime-summary.ps1 `
            -Addr $Addr `
            -TaskName "\TraceDeck\TraceDeck Phase112 Newman" `
            -PidPath $pidPath `
            -ReadyPath $readyPath `
    ...
    Invoke-TraceDeckLoggedCommand -Label "Run Newman Phase 112 collection" -Command {
        newman run ./postman/tracedeck-backend-phase112.postman_collection.json --env-var "baseUrl=$baseUrl" --reporters "cli,json" --reporter-json-export $reportPath
    }
    ...
    Write-TraceDeckLog -Level "INFO" -Message "Newman Phase 112 report: $reportPath"
