...
Initialize-TraceDeckScriptLog -Name "newman-phase19" -LogRoot "logs/local/newman" | Out-Null
...
try {
    ...
    $runRoot = Join-Path $script:TraceDeckRepoRoot "data/local/newman/phase19/$timestamp"
    ...
    $baseUrl = "http://$Addr"
    Invoke-TraceDeckLoggedCommand -Label "Build backend Phase 19 Newman executable" -Command {
    ...
    $backend = Start-Process -FilePath $exePath -ArgumentList @(
        ...
        "--api-key-actor-id", "phase19-newman"
    ...
    Write-TraceDeckLog -Level "INFO" -Message "Started backend for Phase 19 Newman pid=$($backend.Id) addr=$Addr"

    Wait-TraceDeckBackend -BaseUrl $baseUrl

    Invoke-TraceDeckLoggedCommand -Label "Run Newman Phase 19 collection" -Command {
        newman run ./postman/tracedeck-backend-phase19.postman_collection.json --env-var "baseUrl=$baseUrl" --env-var "apiKey=$ApiKey" --reporters "cli,json" --reporter-json-export $reportPath
    }
    ...
    Write-TraceDeckLog -Level "INFO" -Message "Newman Phase 19 report: $reportPath"
    ...
    if ($backend -and -not $backend.HasExited) {
        ...
        Write-TraceDeckLog -Level "INFO" -Message "Stopped backend Phase 19 Newman process: $($backend.Id)"
    }
}
