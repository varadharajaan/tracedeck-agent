...
Initialize-TraceDeckScriptLog -Name "newman-phase16" -LogRoot "logs/local/newman" | Out-Null
...
try {
    ...
    $runRoot = Join-Path $script:TraceDeckRepoRoot "data/local/newman/phase16/$timestamp"
    ...
    $baseUrl = "http://$Addr"
    Invoke-TraceDeckLoggedCommand -Label "Build backend Phase 16 Newman executable" -Command {
    ...
    $backend = Start-Process -FilePath $exePath -ArgumentList @(
        ...
        "--api-key-actor-id", "phase16-newman"
    ...
    Write-TraceDeckLog -Level "INFO" -Message "Started backend for Phase 16 Newman pid=$($backend.Id) addr=$Addr"

    Wait-TraceDeckBackend -BaseUrl $baseUrl

    Invoke-TraceDeckLoggedCommand -Label "Run Newman Phase 16 collection" -Command {
        newman run ./postman/tracedeck-backend-phase16.postman_collection.json --env-var "baseUrl=$baseUrl" --env-var "apiKey=$ApiKey" --reporters "cli,json" --reporter-json-export $reportPath
    }
    ...
    Write-TraceDeckLog -Level "INFO" -Message "Newman Phase 16 report: $reportPath"
    ...
    if ($backend -and -not $backend.HasExited) {
        ...
        Write-TraceDeckLog -Level "INFO" -Message "Stopped backend Phase 16 Newman process: $($backend.Id)"
    }
}
